...
void sin4_intrin(double* sinx, const double* x) {
...
#if defined(__AVX__)
  __m256d x1, x2, x3, x5, x7, x9, x11; // adding more terms to the sin Taylor expansion
  x1  = _mm256_load_pd(x);
  ...
  for (int i = 0; i < 4; i+=sse_length) {
    __m128d x1, x2, x3, x5, x7, x9, x11; // adding more terms to the sin Taylor expansion
    x1  = _mm_load_pd(x+i);
    x2  = _mm_mul_pd(x1, x1);
    x3  = _mm_mul_pd(x1, x2);
    x5  = _mm_mul_pd(x3, x2);
...
void sin4_vector(double* sinx, const double* x) {
  ...
  Vec4 x1, x2, x3, x5, x7, x9, x11; // adding more terms to the sin Taylor expansion
